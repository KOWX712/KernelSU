...
fun SettingScreen(navigator: DestinationsNavigator) {
    ...
    Scaffold(
        ...
        Column(
            ...
            //     summary = stringResource(id = R.string.settings_check_update_summary, color = MaterialTheme.colorScheme.outline),
            //     checked = checkUpdate
            // ) {
            ...
            KsuIsValid() {
                ...
                var avcSpoofMode by rememberSaveable {
                    mutableIntStateOf(
                        run {
                            val currentEnabled = Natives.isAvcSpoofEnabled()
                            val savedPersist = prefs.getInt("avc_spoof_mode", 0)
                            if (savedPersist == 2) 2 else if (!currentEnabled) 1 else 0
                        }
                    )
                }
