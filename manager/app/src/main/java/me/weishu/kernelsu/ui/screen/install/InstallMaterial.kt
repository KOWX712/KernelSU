...
fun InstallScreenMaterial() {
    ...
    val selectLkmLauncher = rememberLauncherForActivityResult(
        ...
        if (it.resultCode == Activity.RESULT_OK) {
            it.data?.data?.let { uri ->
                val isKoFile = isKoFile(context, uri)
                if (isKoFile) {
    ...
    val showKmiDialog = rememberSaveable { mutableStateOf(false) }
    val currentKmi = produceState(initialValue = "") { value = getCurrentKmi() }.value

    ChooseKmiDialog(
        showDialog = showKmiDialog,
        onSelected = { kmi ->
            if (kmi != null) {
    ...
    Scaffold(
        ...
        Column(
            ...
            SelectInstallMethod(onSelected = { installMethod = it })

            val isOta = installMethod is InstallMethod.DirectInstallToInactiveSlot
            val partitions = produceState(initialValue = emptyList<String>()) {
                value = getAvailablePartitions()
            }.value
            val defaultPartition = produceState(initialValue = "boot") {
...
private fun SelectInstallMethod(onSelected: (InstallMethod) -> Unit = {}) {
    ...
    val selectFileTip = stringResource(R.string.select_file_tip, defaultPartitionName)
    val selectFileTipNoGKI = stringResource(R.string.select_file_tip_nogki)

    val radioOptions = mutableListOf<InstallMethod>(
        InstallMethod.SelectFile(summary = if (isGkiDevice) selectFileTip else selectFileTipNoGKI)
    )
